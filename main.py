...
def main():
    ...
    parser.add_argument("--data_dir",
                        default=r'../data/Ptacek',     #Ghosh，    Ptacek，  SARC_politics
                        type=str)
    ...
    parser.add_argument("--output_dir",
                        default=r'../data/Ptacek/output_save',
                        type=str,
                        #required=True,
    ...
    parser.add_argument("--n_head",
                        default=12,
                        type=int,
                        help="The heads of Multihead-attention.")
    ...
    parser.add_argument('--know_strategy',
                        default='common_know.txt',
                        help='know strategy')  # common_know.txt, major_sent_know.txt, minor_sent_know.txt, contrast_sent_know.txt
    ...
    if args.do_train:
        ...
        for train_idx in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            eval_loss = round(eval_loss / nb_eval_steps, 8)
            eval_accuracy = round(eval_accuracy / nb_eval_examples, 8)
            train_loss = round(tr_loss / nb_tr_steps, 8) if args.do_train else None
            ...
            F_score = round(F_score, 8)

            print("***** Dev Eval results *****")
            print(f'epoch: {train_idx + 1}, train_loss: {train_loss}, eval_loss: {eval_loss}, accuracy: {eval_accuracy}, precision: {precision}, recall: {recall}, f_score: {F_score}')

            with open(output_eval_file, "a") as writer:
                curr_time = datetime.datetime.now()
                time_str = datetime.datetime.strftime(curr_time, '%Y-%m-%d %H:%M:%S')
                writer.write(
                    f"time: {time_str}, epoch: {train_idx + 1}, train_loss: {train_loss}, eval_loss: {eval_loss}, accuracy: {eval_accuracy}, precision: {precision}, recall: {recall}, f_score: {F_score}\n")

            if eval_accuracy > max_acc:
                torch.save(model.state_dict(), output_model_file)
                max_acc = eval_accuracy
                best_epoch = train_idx + 1
        print(f'total_time:{time.time() - time_total}')
    ...
    if args.do_test:
        ...
        eval_loss = round(eval_loss / nb_eval_steps, 8)
        eval_accuracy = round(eval_accuracy / nb_eval_examples, 8)
        loss = round(train_loss, 8) if args.do_train else None
        ...
        with open(output_eval_file, "a") as writer:
            ...
            print(f"data_name: {data_name}, know_strategy: {args.know_strategy}, MSL:{args.max_seq_length},  MKL: {args.max_know_length}, best_epoch: {best_epoch}，time: {time_str}, train_b_s: {args.train_batch_size}, lr: {args.learning_rate}, n_tra_epochs: {args.num_train_epochs}\n"
                f" train_loss: {loss}, test_loss: {eval_loss}, accucary: {eval_accuracy}, precision: {precision}, recall: {recall}, f_score: {F_score}\n")

            writer.write(
                f"data_name: {data_name}, know_strategy: {args.know_strategy}, MSL:{args.max_seq_length}, MKL: {args.max_know_length}, best_epoch: {best_epoch}，time: {time_str}, train_batch_size: {args.train_batch_size}, learning_rate: {args.learning_rate}, num_train_epochs: {args.num_train_epochs}\n"
                f"train_loss: {loss}, test_loss: {eval_loss}, accucary: {eval_accuracy}, precision: {precision}, recall: {recall}, f_score: {F_score}, data_path: {args.data_dir}\n")
